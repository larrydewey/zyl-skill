...
| Traits | static resolution, bounds, coherence | static resolution from inferred receiver types (runtime tag match only when unknown); `trait` signatures type calls; prelude `Show` and `Secret`; `impl-not` (`E_IMPL_FORBIDDEN`, with flow rule); orphan rule enforced; C1 fails in assembler; no bounds, defaults, dyn, assoc types |
...
| Secrets | (implementation-defined) | syntactic taint checker (impl bodies included); `Secret` fields and `Secret`-trait types, redacted as `<secret>` by `Show`; frames holding secrets zeroed on return (no tail calls); heap erasure manual (`zeroize`/`wipe`); annotations stop at `math/secret/secret`; `set!` into `let-mut` not tracked |
