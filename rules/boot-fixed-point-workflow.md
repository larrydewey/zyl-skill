...
```
0. copies stdlib/ and actor_runtime.c into build/boot/  (what every stage resolves)
...
6. writes the zyl-self wrapper and builds zyl-lsp in build/boot/
```

Comparisons are `cmp` on **assembly**. Each stage runs under `ZYL_STAGE_TIMEOUT` (default 2400 s) and an allocation ceiling `ZYL_STAGE_MEMORY` (default 2 GB, passed on as `ZYL_MAX_MEMORY`); a self-compile needs about 1.4 GB, so `E_OUT_OF_MEMORY` in a stage means a compiler change allocates far more than before ([pass-no-allocation-in-lookups](pass-no-allocation-in-lookups.md)). A full verification takes well under a minute. `boot.sh` exports `ZYL_HOME=build/boot`.
