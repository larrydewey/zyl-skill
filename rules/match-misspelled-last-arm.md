...
The compiler treats *any* identifier in arm-head position that is not a known constructor as a catch-all, with or without field binders. Anywhere but the last arm this is caught (`E_UNREACHABLE_MATCH_ARM`, because arms follow a catch-all). **In the last arm it silently matches everything the earlier arms did not.** Its "field binders" bind nothing, so an arm that uses one fails with `E_UNBOUND_VARIABLE` (a useful hint); a nullary misspelling or an arm that ignores its binders compiles without a word. The same happens when a constructor is unknown inside a module because the module did not `use` the file defining it.
...
(deftype Sh (Sq Int) (Ci Int))
(defn r (s) (match s (Sq a a) (Circl x x)))   ; E_UNBOUND_VARIABLE `x`: the typo shows
```
