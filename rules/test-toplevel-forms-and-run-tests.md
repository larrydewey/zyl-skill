...
(test "vector-push"
  (assert-equal (vec-len (vec-push (vec-create 0 4) 42)) 1))
...
- Test bodies may hold several forms (evaluated in order; `begin` is equivalent). The first failing assertion ends that test; other tests still run.
