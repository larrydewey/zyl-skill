...
A handle is typed: `bytebuf` returns `ByteBuf`, `byteslice`/`byteslice-sub` return `ByteSlice`, and passing an `Int`/`Float`/`Bool`/`String` where a handle is expected is `E_TYPE_MISMATCH`. A parameter used only by loads and stores must be annotated, or it is `E_CANNOT_INFER` (either handle would do).

## Bad

```lisp
(defn load-u32-le (buf off)                    ; hand-rolled, four calls; and
  (bit-or (load-u8 :le buf off)                ; E_CANNOT_INFER: buf is never annotated
          (shl (load-u8 :le buf (+ off 1)) 8)
          (shl (load-u8 :le buf (+ off 2)) 16)
          (shl (load-u8 :le buf (+ off 3)) 24)))
(load-u16 :le 7 0)                             ; E_TYPE_MISMATCH: expected `ByteBuf`, found `Int`
```
...
(load-i16 :le buf 8)                           ; -2
(load-u64 :le buf 8)                           ; -2: the bit pattern as an Int
```

## Notes

- Implementation: the width rides in the `Endian` value (`EWide`), lowered to `zyl_load_n`/`zyl_load_n_signed`/`zyl_store_n`. Only the one-byte forms are inlined by the native backend; the wide forms are runtime calls.
- The filename predates the wide forms; the rule is now about using them.
