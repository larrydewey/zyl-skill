...
- Lowering: a `zyl_*` symbol is trusted runtime code, called directly as `IFfi sym args`; its timeout is checked but unused. The test is the prefix alone, so your own C function named `zyl_...` is called directly too, with no timeout: give it another prefix. Any other symbol becomes `IFfi "zyl_ffi_timed" (ISymAddr sym, IStr sym, IConst ms, IConst argc, args...)`.
